...
def _attn_fwd(Q, K, V, sm_scale, M, Out,  #
    ...
    """
        q shape: [batch_size, num_head, seqlen, head_dim], namely [B, N, S, H]
        grid = lambda args: (triton.cdiv(q.shape[2], args["BLOCK_M"]), q.shape[0] * q.shape[1], 1), namely (S/BLOCK_M, B*N, 1)
        M = torch.empty((q.shape[0], q.shape[1], q.shape[2]), device=q.device, dtype=torch.float32), namely [B, N, S]
    """
    tl.static_assert(BLOCK_N <= HEAD_DIM)
    start_m = tl.program_id(0)  # S/BLOCK_M
    off_hz = tl.program_id(1)  # B*N
    off_z = off_hz // N  # batch_size
    off_h = off_hz % N  # num_head
    qvk_offset = off_z.to(tl.int64) * stride_qz + off_h.to(tl.int64) * stride_qh  # BN index
    ...
    # stage 1: off-band
    # For causal = True, STAGE = 3 and   gets 1 as its STAGE
